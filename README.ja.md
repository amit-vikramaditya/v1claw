...
<h1>V1Claw: Go で書かれた超効率 AI アシスタント</h1>
...
🦐 V1Claw は [nanobot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。Go でゼロからリファクタリングされ、AI エージェント自身がアーキテクチャの移行とコード最適化を推進するセルフブートストラッピングプロセスで構築されました。
...
## 📢 ニュース
2026-02-09 🎉 V1Claw リリース！$10 ハードウェアで 10MB 未満の RAM で動く AI エージェントを 1 日で構築。🦐 皮皮虾，我们走！
...
### 🐜 革新的な省フットプリントデプロイ
V1Claw はほぼすべての Linux デバイスにデプロイできます！
...
[リリースページ](https://github.com/amit-vikramaditya/v1claw/releases) からお使いのプラットフォーム用のファームウェアをダウンロードしてください。
...
Docker Compose を使えば、ローカルにインストールせずに V1Claw を実行できます。
...
> [!TIP]
> `~/.v1claw/config.json` に API キーを設定してください。
...
Telegram、Discord、QQ、DingTalk、LINE で V1Claw と会話できます
...
> **Docker Compose**: `v1claw-gateway` サービスに `ports: ["18791:18791"]` を追加して Webhook ポートを公開してください。
...
V1Claw は設定されたワークスペース（デフォルト: `~/.v1claw/workspace`）にデータを保存します：
...
V1Claw はデフォルトでサンドボックス環境で実行されます。エージェントは設定されたワークスペース内のファイルにのみアクセスし、コマンドを実行できます。
...
V1Claw は自動的に定期タスクを実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成します：
...
**環境変数:**
- `V1CLAW_HEARTBEAT_ENABLED=false` で無効化
- `V1CLAW_HEARTBEAT_INTERVAL=60` で間隔変更
...
別のインスタンスが実行中の場合に発生します。`v1claw gateway` が 1 つだけ実行されていることを確認してください。
