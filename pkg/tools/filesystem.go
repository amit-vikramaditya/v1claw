...
const defaultMaxWriteBytes = 50 * 1024 * 1024 // 50 MB
...
func (t *WriteFileTool) Execute(ctx context.Context, tc ToolContext, args map[string]interface{}) *ToolResult {
	...
	if len(content) > defaultMaxWriteBytes {
		return ErrorResult(fmt.Sprintf("content too large to write (max %d bytes)", defaultMaxWriteBytes))
	}
