...
func TestConvertConfig(t *testing.T) {
	t.Run("providers mapping", func(t *testing.T) {
		...
		if cfg.Providers.Nvidia.APIKey != "sk-nv-test" {
			t.Errorf("Nvidia.APIKey = %q, want %q", cfg.Providers.Nvidia.APIKey, "sk-nv-test")
		}
	})
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		data := map[string]interface{}{
			"providers": map[string]interface{}{
				"customllm": map[string]interface{}{
		...
		if warnings[0] != "Provider 'customllm' not supported in V1Claw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in V1Claw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
...
func TestRunDryRun(t *testing.T) {
	...
	picoWs := filepath.Join(v1ClawHome, "workspace")
	if _, err := os.Stat(filepath.Join(picoWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(v1ClawHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	picoConfig, err := config.LoadConfig(filepath.Join(v1ClawHome, "config.json"))
	if err != nil {
		t.Fatalf("loading V1Claw config: %v", err)
	}
