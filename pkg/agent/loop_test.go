...
// TestInferSameProviderFallback verifies Layer 1 auto-resilience model mapping.
func TestInferSameProviderFallback(t *testing.T) {
	cases := []struct {
		primary  string
		expected string
	}{
		...
		{"claude-3-5-sonnet-20241022", "claude-haiku-3-5"},
		{"claude-haiku-3-5", ""},
		// OpenAI
		{"gpt-4o", "gpt-4o-mini"},
	...
	for _, tc := range cases {
		got := inferSameProviderFallback(tc.primary)
		if got != tc.expected {
			t.Errorf("inferSameProviderFallback(%q) = %q, want %q", tc.primary, got, tc.expected)
		}
	}
}
